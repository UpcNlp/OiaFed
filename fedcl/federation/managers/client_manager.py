...
class ClientManager:
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        # 配置参数
        self.max_clients = config.get("max_客户端", 100)
        self.client_timeout = config.get("client_timeout", 300)  # 5分钟
        self.max_failures = config.get("max_failures", 3)
        
        logger.debug(f"Initialized ClientManager with {self.selection_strategy} selection strategy")
    ...
    def register_client(self, client_id: str, client_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(f"Client {client_id} registered successfully")
            return True
    ...
    def unregister_client(self, client_id: str) -> bool:
        ...
        try:
            ...
            logger.debug(f"Client {client_id} unregistered successfully")
            return True
    ...
    def select_clients_for_round(self, num_clients: int, round_id: int) -> List[str]:
        ...
        logger.debug(f"选择 {len(selected)} clients for round {round_id}: {selected}")
        return selected
    ...
    def broadcast_to_clients(self, message: Any, targets: List[str]) -> Dict[str, bool]:
        ...
        if self.communication_manager is None:
            # 伪联邦模式，模拟广播
            logger.debug(f"Pseudo-federation mode: simulating broadcast to {len(targets)} 客户端")
        ...
        logger.debug(f"Broadcast completed: {sum(results.values())}/{len(targets)} successful")
        return results
    ...
    def collect_from_clients(self, data_type: str, sources: List[str], 
        ...
        if self.communication_manager is None:
            # 伪联邦模式，模拟数据收集
            logger.debug(f"Pseudo-federation mode: simulating data collection from {len(sources)} 客户端")
        ...
        logger.debug(f"Data collection completed: {len(collected_data)}/{len(sources)} successful")
    ...
    def set_client_selection_strategy(self, strategy: str) -> None:
        ...
        logger.debug(f"Client selection strategy updated to: {strategy}")
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        return {
            "total_客户端": total_clients,
            "active_客户端": active_clients,
