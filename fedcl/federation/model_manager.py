...
from ..core.exceptions import ConfigurationError, ModelStateError
...
class ModelManager:
    ...
    def update_global_model(self, client_updates: List[Dict[str, torch.Tensor]]) -> torch.nn.Module:
        ...
        Args:
            client_updates: 客户端更新列表，每个更新包含参数差异
        ...
        try:
            ...
            logger.info(f"Global model updated with {len(client_updates)} client updates")
            return self.global_model
    ...
    def set_global_model(self, model: torch.nn.Module) -> None:
        ...
        try:
            ...
            logger.info("Global model set successfully")
    ...
    def load_checkpoint(self, checkpoint_path: Path) -> torch.nn.Module:
        ...
        try:
            ...
            logger.info(f"Restored from round {checkpoint_data.get('round_id', 'unknown')}")
            ...
            for name in update:
                ...
                if update[name].shape != model_params[name].shape:
                    logger.warning(f"Shape mismatch for parameter {name}: "
                                 f"expected {model_params[name].shape}, "
                                 f"got {update[name].shape}")
                    return False
