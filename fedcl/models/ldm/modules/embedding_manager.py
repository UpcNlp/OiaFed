...
from fedcl.models.ldm.data.personalized import per_img_token_list
...
def get_bert_token_for_string(tokenizer, string):
    ...
    if torch.count_nonzero(token) > 3:
        print(f"Warning: String '{string}' maps to {torch.count_nonzero(token)} tokens, using first non-zero token")
        # 找到第一个非零token
        non_zero_indices = torch.nonzero(token[0])
        if len(non_zero_indices) > 0:
            # 修复索引问题：non_zero_indices[0]是一个一维张量，直接取第一个元素
            token = token[0, non_zero_indices[0]]
        else:
            token = token[0, 1]
    else:
        token = token[0, 1]
