...
from ..base import TrainingResult, EvaluationResult
from ...registry import registry
from ...api.decorators import trainer


@trainer("default", description="标准联邦训练器 - FedAvg实现")
class StandardFederationTrainer(BaseTrainer):
    """
    标准联邦训练器 - FedAvg算法实现
    ...
    def setup_training(self, **kwargs) -> None:
        ...
        self.update_global_state("current_round", 0)
        logger.info("🔧 标准联邦训练器训练环境已设置")
    ...
    def execute_client_round(self, round_num: int, client_ids: list, global_model_weights: dict = None, **kwargs) -> list:
        ...
        global_weights = global_model_weights or self.get_global_state("global_model_weights", {})
        return self._execute_client_training(client_ids, round_num, global_weights, **kwargs)
    
    def execute_server_aggregation(self, client_results: list, round_num: int = None, **kwargs) -> dict:
        """执行服务端聚合（与装饰器约定保持一致）"""
        return self._execute_aggregation(client_results)
